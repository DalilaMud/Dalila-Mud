...
ACMD(do_action)
{
  ...
  if (!(vict = get_char_room_vis(ch, buf))) {
    ...
    if (GET_POS(vict) < action->min_victim_position)
      ...
      if (cmd == find_command("schiaffeggia") || cmd == find_command("slap"))
        if (affected_by_spell(vict, SPELLSKILL, DISEASE_MORFEO) && GET_POS(vict) == POS_SLEEPING){
          if (!number(0, 99)){
            act ("La tua salutare cura a base di ceffoni fa riprendere $N dal suo stato di sonnolenza!", FALSE, ch, 0, vict, TO_CHAR);
            act ("Che dolore! La tua guancia e' in fiamme... ma sembra che ogni segno di stanchezza abbia abbandonato il tuo corpo.", FALSE, ch, 0, vict, TO_VICT | TO_SLEEP);
            act ("Grazie al delicato trattamento di $n, $N si riprende dalla sua malattia onirica.", FALSE, ch, 0, vict, TO_NOTVICT);
            affect_from_char (vict, SPELLSKILL, DISEASE_MORFEO);
