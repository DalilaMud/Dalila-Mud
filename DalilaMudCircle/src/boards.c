...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {21623, 0, 0, LVL_IMMORT, "etc/cyrel_sociale" , 0},
 // {2414859, 0, 0, LVL_IMMORT, "etc/cyr.newbie"  , 0},
//  {21627, 0, 0, LVL_IMMORT, "etc/cyr.mestieri", 0},
  {21624, 0, 0, LVL_IMMORT, "etc/lamorka_sociale"   , 0},
//  {490  , 0, 0, LVL_IMMORT, "etc/lam.sociale" , 0},
//  {497  , 0, 0, LVL_IMMORT, "etc/lam.newbie"  , 0},
//  {498  , 0, 0, LVL_IMMORT, "etc/lam.mestieri", 0},
//  {499  , 0, 0, LVL_IMMORT, "etc/lam.porto"   , 0},
//  {12556, 0, 0, LVL_IMMORT, "etc/deira.sociale"   , 0},
//  {12557, 0, 0, LVL_IMMORT, "etc/deira.newbie"    , 0},
  {12559, 0, 0, LVL_IMMORT, "etc/deira_sociale"  , 0},
//  {12560, 0, 0, LVL_IMMORT, "etc/deira.mestieri"  , 0},
//  {12561, 0, 0, LVL_IMMORT, "etc/deira.architetti", 0},
  {9733 , 0, 0, LVL_IMMORT, "etc/cimmura_sociale" , 0},
  {3096, LVL_IMMORT, LVL_IMMORT, LVL_GRGOD, "etc/board.immort", 0},
  {3098, LVL_IMMORT, LVL_FREEZE, LVL_IMPL, "etc/board.freeze", 0},
  {50045, 0, 0, LVL_IMMORT, "etc/alorn_sociale", 0}
  };
...
int CMD_READ, CMD_LOOK, CMD_EXAMINE, CMD_WRITE, CMD_REMOVE,CMD_LEGGI, CMD_GUARDA, CMD_ESAMINA, CMD_SCRIVI, CMD_RIMUOVI;
...
void init_boards(void)
{
  ...
  CMD_RIMUOVI = find_command("rimuovi");
  CMD_ESAMINA = find_command("esamina");
  CMD_GUARDA = find_command("guarda");
  CMD_LEGGI = find_command("leggi");
  CMD_SCRIVI = find_command("scrivi");
...
SPECIAL(gen_board)
{
  ...
  if (cmd != CMD_WRITE && cmd != CMD_LOOK && cmd != CMD_EXAMINE &&
      cmd != CMD_READ && cmd != CMD_REMOVE && cmd != CMD_RIMUOVI && cmd != CMD_LEGGI && cmd != CMD_SCRIVI && cmd != CMD_ESAMINA && cmd != CMD_GUARDA)
    return 0;
  ...
  if (cmd == CMD_WRITE || cmd == CMD_SCRIVI) {
    Board_write_message(board_type, ch, argument);
    return 1;
  } else if (cmd == CMD_LOOK || cmd == CMD_EXAMINE || cmd == CMD_GUARDA || cmd == CMD_ESAMINA)
    return (Board_show_board(board_type, ch, argument));
  else if (cmd == CMD_READ || cmd == CMD_LEGGI)
    return (Board_display_msg(board_type, ch, argument));
  else if (cmd == CMD_REMOVE || cmd == CMD_RIMUOVI)
...
int crime_reward(struct char_data *ch, int clan){
  ...
  if (!IS_NPC(ch) && (clan<=0)) return 0;
  
  if(!IS_NPC(ch)){
    if(FLAGGED_CRIME(ch,clan,CLAN_KILLER)){
      crime=5000;
      taglia+=MAX(crime,crime*(ch->player_specials->saved.num_kills));
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_TKILLER)){
      crime=300;
      taglia+=MAX(crime,crime*(ch->player_specials->saved.num_tkills));
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_THIEF)){
      crime=200;
      taglia+=MAX(crime,crime*(ch->player_specials->saved.num_thief));
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_WKILLER)){
      crime=300;
      taglia+=MAX(crime,crime*(ch->player_specials->saved.num_warkills));
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_STRAGE)){
      crime=300;
      taglia+=MAX(crime,crime*(ch->player_specials->saved.num_kills));
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_CORROTTO)){
      crime=300;
      taglia+=crime;
    }
    if(FLAGGED_CRIME(ch,clan,CLAN_MULTA)){
      crime=100;
      taglia+=crime;
    }
  }
...
int notorieta_reward(struct char_data *ch,struct char_data *victim){
  ...
  int mod=0;
  int notorieta = GET_NOTORIETA(ch);
  
  if (victim) {  
   if(!IS_NPC(victim)) {
    mod= 0.05 * victim->player_specials->saved.num_tkills+
      0.2*(victim->player_specials->saved.num_thief+
	   victim->player_specials->saved.num_wanted)+
      0.3*(victim->player_specials->saved.num_warkills)+
      0.5*(victim->player_specials->saved.num_kills);
	notorieta += (int) (GET_NOTORIETA(victim) * mod/100) + 1;  
   }	  
   
   if(IS_NPC(victim)){
     if (MOB_FLAGGED(victim,MOB_CRIMINAL))
       notorieta = (int) GET_LEVEL(victim) / 5 + 1;
 	 else  
  	   notorieta= - (int)(GET_LEVEL(victim) / 5 + 1);
    }
  }
...
SPECIAL(crime_board)
{
  int i,j,kills,tkills=0,warkills=0,thief=0,wanted=0,taglia=0,counter;
  extern struct player_index_element *player_table;
  extern int top_of_p_table;
  struct char_file_u chdata;
  register struct char_data *mob=NULL;
  char line_disp[90];
  ...
  CMD_READ = find_command("read");
  CMD_LEGGI = find_command("leggi");
  if(cmd != CMD_READ && cmd != CMD_LEGGI)
    return 0;
	
  strcpy(megabuffer, "\r\n");	
  send_to_char("\t&4Bollettino dei Ricercati&0\n",ch);
  send_to_char("-------------------------------------------------------\n",ch);
  for (j = 0; j <= top_of_p_table; j++){
    if((victim=is_playing((player_table +j)->name))) {
	if(PLR_FLAGGED(victim,PLR_KILLER)||PLR_FLAGGED(victim,PLR_TKILLER)
	   ||PLR_FLAGGED(victim,PLR_THIEF)||PLR_FLAGGED(victim,PLR_WANTED)
	   ||PLR_FLAGGED(victim,PLR_WAR_KILLER))  {
	  found=TRUE;
	  counter=12 -strlen(GET_NAME(victim));
	  sprintf(line_disp,"%s",GET_NAME(victim));
	  for(i=0;i<=counter;i++)
	    strcat(line_disp," ");
	  strcat(line_disp,":");
	  counter=0;
	  if (PLR_FLAGGED(victim, PLR_THIEF)){
	    strcat(line_disp, "&2THIEF &0");
	    counter++;
	  }
	  if (PLR_FLAGGED(victim, PLR_KILLER)){
	    strcat(line_disp, "&1KILLER &0");
	    counter++;
	}
	  if (PLR_FLAGGED(victim, PLR_TKILLER)){
	    strcat(line_disp, "&6TKILLER &0");
	    counter++;
	}
	  if (PLR_FLAGGED(victim, PLR_WAR_KILLER)){
	    strcat(line_disp, "&5WKILLER &0");
	  counter++;
	  }
	  if (PLR_FLAGGED(victim, PLR_WANTED)){
	    strcat(line_disp, "&4WANTED &0");
	  counter++;
	  }
	  for(i=0;strlen(line_disp)<=(46+4*counter);i++)
	    strcat(line_disp," ");
	  taglia=crime_reward(victim,0);
	  sprintf(buf,"&3Monete&0 %d\n",taglia);
	  strcat(line_disp,buf);
      ...
      if(((kills=chdata.player_specials_saved.num_kills)>0)
	 ||((tkills=chdata.player_specials_saved.num_tkills)>0)
	 ||((thief=chdata.player_specials_saved.num_thief)>0)
	 ||((warkills=chdata.player_specials_saved.num_warkills)>0)
	 ||((wanted=chdata.player_specials_saved.num_wanted)>0)){
	found=TRUE;
	counter=12 -strlen(chdata.name);
	sprintf(line_disp,"%s",chdata.name);
	for(i=0;i<=counter;i++)
	  strcat(line_disp," ");
	strcat(line_disp,":");
	counter=0;
	...
	for(i=0;strlen(line_disp)<=(46+4*counter);i++)
	  strcat(line_disp," ");
	taglia=10000*kills+500*(wanted+tkills+warkills+thief);
	sprintf(buf,"&3Monete&0 %d\n",taglia);
	strcat(line_disp,buf);
	strcat(megabuffer,line_disp);
	//send_to_char(line_disp,ch);
      }
    }
  }
    for (mob = character_list; mob; mob = mob->next){
      if(IS_NPC(mob)){
	if(MOB_FLAGGED(mob,MOB_CRIMINAL)){
	  found=TRUE;
	  all_alias = strdup(mob->player.name);
	  name=strtok(all_alias,WHITESPACE);
	  counter=12 -strlen(name);
	  sprintf(line_disp,"%s",name);
	  for(i=0;i<=counter;i++)
	    strcat(line_disp," ");
	  strcat(line_disp,":");
	  counter=0;
    ...
    if(found==TRUE)
      send_to_char("\n&4&bLE RICOMPENSE VERRANNO PAGATE AL MOMENTO DELLA CONSEGNA&0\n",ch);
    else
      send_to_char("\n              &1&bNESSUN RICERCATO AL MOMENTO&0\n",ch);
    send_to_char(  "\n-------------------------------------------------------\n",ch);
    return 1;
...
void Board_write_message(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (GET_LEVEL(ch) < WRITE_LVL(board_type)) {
    send_to_char("Non sei grande abbastanza per farlo.\r\n", ch);
    return;
  }
  ...
  if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1) {
    send_to_char("La bacheca non funziona-contata un Creatore!.\r\n", ch);
  ...
  if (!*arg) {
    send_to_char("Devi avere un soggetto!\r\n", ch);
    return;
  }
  ct = time(0);
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  if (!(NEW_MSG_INDEX(board_type).heading = (char *) malloc(sizeof(char) * len))) {
    send_to_char("La bacheca non funziona contatta un creatore.\r\n", ch);
    return;
  }
  ...
  send_to_char("Scrivi il tuo messaggio.  (/s per salvarlo /h per un aiuto)\r\n\r\n", ch);
  act("$n scrive un messaggio.", TRUE, ch, 0, 0, TO_ROOM);
...
int Board_show_board(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (!*tmp || !isname(tmp, "board bulletin bacheca"))
    return 0;

  if (GET_LEVEL(ch) < READ_LVL(board_type)) {
    send_to_char("Ci tenti ma non sai leggere!!\r\n", ch);
    return 1;
  }
  act("$n studia la bacheca.", TRUE, ch, 0, 0, TO_ROOM);

  strcpy(buf,
	 "Questa e' una bacheca. Usa: READ/REMOVE <messagg #>, WRITE <soggetto>.\r\n"
	 "                    Oppure: LEGGI/RIMUOVI <messagg #>, SCRIVI <soggetto>.\r\n"
	 "Riesamina la bacheca per salvare il tuo mesaggio.\r\n");
  if (!num_of_msgs[board_type])
    strcat(buf, "La bacheca e' vuota.\r\n");
  else {
    sprintf(buf + strlen(buf), "Ci sono %d messaggi sulla bacheca.\r\n",
	    num_of_msgs[board_type]);
...
/*    for (i = num_of_msgs[board_type] - 1; i >= 0; i--) { */
	...
	send_to_char("Scusa, la bacheca non e' in funzione.\r\n", ch);
	return 1;
      }
    }
  }
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (!*number)
    return 0;
  if (isname(number, "board bulletin bacheca"))	/* so "read board" works e anche "read bacheca"*/
    return (Board_show_board(board_type, ch, arg));
  if (!isdigit(*number) || (!(msg = atoi(number))))
    return 0;

  if (GET_LEVEL(ch) < READ_LVL(board_type)) {
    send_to_char("Tenti di farlo ma non riesci a capire niente.\r\n", ch);
    return 1;
  }
  if (!num_of_msgs[board_type]) {
  ...
  if (msg < 1 || msg > num_of_msgs[board_type]) {
    send_to_char("Questo messaggio esiste solo nella tua immaginazione.\r\n",
		 ch);
    return (1);
  }
  ind = msg - 1;
  ...
  if (!(MSG_HEADING(board_type, ind))) {
    send_to_char("Questo messaggio sembra essere rovinato.\r\n", ch);
    return 1;
  }
  if (!(msg_storage[MSG_SLOTNUM(board_type, ind)])) {
    send_to_char("QUesto messaggio sembra essere vuoto.\r\n", ch);
    return 1;
  }
...
int Board_remove_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  ind = msg - 1;
  if (!MSG_HEADING(board_type, ind)) {
    send_to_char("Il messaggio sembra rovinato.\r\n", ch);
    return 1;
  }
  sprintf(buf, "(%s)", GET_NAME(ch));

  if (   BOARD_VNUM(board_type) != 21623
      && BOARD_VNUM(board_type) != 21624
      && BOARD_VNUM(board_type) != 12559
      && BOARD_VNUM(board_type) != 9733
      ) {

    if (GET_LEVEL(ch) < REMOVE_LVL(board_type) &&
        !(strstr(MSG_HEADING(board_type, ind), buf))) {
      send_to_char("Non puoi rimuovere il messaggio.\r\n", ch);
      return 1;
    }
  ...
  else if (BOARD_VNUM(board_type) == 21623) {
  // Elenia remove permission
    if (   GET_HOME(ch) != 4
        && GET_CLAN_RANK(ch) < 23) {
      send_to_char("Non hai il permesso di cancellare messaggi su questa bacheca.", ch);
      return 1;
    }    
  }
  
  else if (BOARD_VNUM(board_type) == 21624) {
  // Sendaria remove permission
    if (   GET_HOME(ch) != 5
        && GET_CLAN_RANK(ch) < 23) {
      send_to_char("Non hai il permesso di cancellare messaggi su questa bacheca.", ch);
      return 1;
  ...
  else if (BOARD_VNUM(board_type) == 12559) {
  // Aldaria remove permission
    if (   GET_HOME(ch) != 1
        && GET_CLAN_RANK(ch) < 23) {
      send_to_char("Non hai il permesso di cancellare messaggi su questa bacheca.", ch);
      return 1;
  ...
  else if (BOARD_VNUM(board_type) == 9733) {
  // Tolnedra remove permission
    if (   GET_HOME(ch) != 3
        && GET_CLAN_RANK(ch) < 23) {
      send_to_char("Non hai il permesso di cancellare messaggi su questa bacheca.", ch);
      return 1;
  ...
  else if (BOARD_VNUM(board_type) == 50045) {
  // Alorn remove permission
    if (   GET_HOME(ch) != 6
        && GET_CLAN_RANK(ch) < 23) {
      send_to_char("Non hai il permesso di cancellare messaggi su questa bacheca.", ch);
      return 1;
  ...
  for (d = descriptor_list; d; d = d->next)
    if (!d->connected && d->str == &(msg_storage[slot_num])) {
      send_to_char("Almeno aspetta che l'autore l'abbia completato!\r\n", ch);
      return 1;
    }
  ...
  num_of_msgs[board_type]--;
  send_to_char("Messaggio rimosso.\r\n", ch);
  sprintf(buf, "$n ha rimosso un messaggio %d.", msg);
