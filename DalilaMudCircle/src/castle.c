...
int banzaii(struct char_data * ch)
{
  ...
  if ((chOpponent = get_victim(ch))) {
    act("$n urla: 'Proteggete il grande Re Welmar!  BANZAIIII!!!'",
...
int do_npc_rescue(struct char_data * ch_hero, struct char_data * ch_victim)
{
  ...
  if (ch_bad_guy) {
    ...
    act("Vai in soccorso di $N.\r\n", FALSE, ch_hero, 0, ch_victim, TO_CHAR);
    act("Sei stato soccorso sa $N, un vero amico!\r\n",
	FALSE, ch_victim, 0, ch_hero, TO_CHAR);
    act("$n eroicamente va in soccorso di $N.", FALSE, ch_hero, 0, ch_victim, TO_NOTVICT);
	          ...
	          int iProhibited_direction)
{
  ...
  if ((ch->in_room == real_room(iIn_room)) && (cmd == iProhibited_direction)) {
    if (!member_of_staff(ch))
      act("La guardia guarda male $n e lo spinge indietro.",
	  FALSE, ch, 0, 0, TO_ROOM);
    send_to_char("La guardia dice: 'L'entrata e' proibita!', e ti spinge indietro.\r\n", ch);
    return (TRUE);
  }
  return FALSE;
}
...
void fry_victim(struct char_data * ch)
{
  ...
  case 2:
  case 3:
    act("Alzi la mano con un drammatico gesto.", 1, ch, 0, 0, TO_CHAR);
    act("$n alza la mano con un drammatico gesto.", 1, ch, 0, 0, TO_ROOM);
    cast_spell(ch, tch, 0, SPELL_COLOR_SPRAY);
    break;
  case 4:
  case 5:
    act("Ti concentri e pensi.", 1, ch, 0, 0, TO_CHAR);
    act("$n si concentra e pensa.", 1, ch, 0, 0, TO_ROOM);
    cast_spell(ch, tch, 0, SPELL_HARM);
    break;
  case 6:
  case 7:
    act("Sembri sprofondato negli occhi di $N.", 1, ch, 0, tch, TO_CHAR);
    act("$n sembra sprofondato negli occhi di $N.", 1, ch, 0, tch, TO_NOTVICT);
    act("L'immagine del tuo corpo riflessa negli occhi di $n sembra bruciare.", 1, ch, 0, tch, TO_VICT);
...
SPECIAL(king_welmar)
{
  ...
  static char *monolog[] = {
    "$n proclama 'Primus in regnis Geticis coronam'.",
    "$n proclama 'regiam gessi, subiique regis'.",
    "$n proclama 'munus et mores colui sereno'.",
    "$n proclama 'principe dignos'."};
  ...
  case 'W':
    ...
    act("$n e' sveglio e ritto in piedi.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'S':
    GET_POS(ch) = POS_SLEEPING;
    act("$n si sdraia nel suo letto e si addormenta.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'r':
    GET_POS(ch) = POS_SITTING;
    act("$n si siede sul suo regale trono.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 's':
    GET_POS(ch) = POS_STANDING;
    act("$n si alza in piedi.", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'G':
    act("$n dice 'Buon giorno, miei fidati amici.'", FALSE, ch, 0, 0, TO_ROOM);
    break;

  case 'g':
    act("$n dice 'Buon giorno, buon soggetto.'", FALSE, ch, 0, 0, TO_ROOM);
    break;
...
SPECIAL(training_master)
{
  ...
  if (!banzaii(ch) && !number(0, 2)) {
	...
	pupil2 = tch;
      ...
      switch (number(0, 7)) {
      case 0:
	act("$n colpisce $N sulla testa.",
	    FALSE, pupil1, 0, pupil2, TO_NOTVICT);
	act("Colpisci $N sulla testa.",
	    FALSE, pupil1, 0, pupil2, TO_CHAR);
	act("$n ti colpisce in testa.",
